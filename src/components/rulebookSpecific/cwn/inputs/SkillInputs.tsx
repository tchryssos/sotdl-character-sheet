...
import { CWN_SKILL_UNTRAINED } from '~/constants/cwn/form';
import { Skill, SKILLS } from '~/constants/cwn/game';
...
function SkillInput({ skill, hideUntrained }: SkillInputProps) {
  ...
  const showUntrained =
    (skillValue === CWN_SKILL_UNTRAINED ||
      String(skillValue) === String(CWN_SKILL_UNTRAINED)) &&
    !isEditMode;
...
export function SkillInputs() {
  ...
  return (
    <FormSection columns={1} icon={RpgIcons.Card} title="Skills">
      ...
      <GridBox columns={skillColumns}>
        <NumberInput<CwnCharacterData> name="remaining_skill_points" />
      </GridBox>
    </FormSection>
  );
}
